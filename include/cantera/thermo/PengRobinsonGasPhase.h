...
#ifndef CT_PENGROBINSONGASPHASE_H
#define CT_PENGROBINSONGASPHASE_H
 ...
 *  description of the specification of reference state species thermodynamics
 functions).
 *  The reference state,
 ...
 *  This class is optimized for speed of execution. All calls to thermodynamic
 functions
 *  first call internal routines (aka #enthalpy_RT_ref()) which return
 references
 *  the reference state thermodynamics functions. Within these internal
 reference
 *  state functions, the function #_updateThermo() is called, that first checks
 to see
 *  whether the temperature has changed. If it has, it updates the internal
 reference
 ...
 *       \f]
 *
 *  The standard state constant-pressure heat capacity is independent of
 pressure:
 *
 *       \f[
 ...
 *       \f]
 *
 *  The standard state internal energy is obtained from the enthalpy function
 also
 *
 *       \f[
 ...
 *
 *       \f[
 *            \tilde{s}_k(T,P) = s^o_k(T,P) - R \log(X_k) = s^{ref}_k(T) - R
 \log(\frac{P X_k}{P_{ref}})
 *       \f]
 *
 ...
 *   The activity concentration,\f$  C^a_k \f$,is given by the following
 expression.
 *
 *       \f[
 ...
 *
 *   \f[
 *         \frac{a_j a_k}{ a_l} = K_a^{o,1} = \exp(\frac{\mu^o_l - \mu^o_j -
 \mu^o_k}{R T} )
 *   \f]
 *
 *  \f$  K_a^{o,1} \f$ is the dimensionless form of the equilibrium constant,
 associated with
 *  the pressure dependent standard states \f$ \mu^o_l(T,P) \f$ and their
 ...
 *
 *   \f[
 *       K_a^{o,1} = \exp(\frac{\mu^{ref}_l - \mu^{ref}_j - \mu^{ref}_k}{R T} )
 * \frac{P_{ref}}{P}
 *   \f]
 *
 ...
 *             \exp(\frac{\mu^{ref}_l - \mu^{ref}_j - \mu^{ref}_k}{R T} ) *
 \frac{P_{ref}}{RT}
 *   \f]
 *
 *    %Kinetics managers will calculate the concentration equilibrium constant,
 \f$ K_c \f$,
 *    using the second and third part of the above expression as a definition
 for the concentration
 *    equilibrium constant.
 *
 *    For completeness, the pressure equilibrium constant may be obtained as
 well
 *
 *   \f[
 *         \frac{P_j P_k}{ P_l P_{ref}} = K_p^1 =
               \exp\left(\frac{\mu^{ref}_l - \mu^{ref}_j - \mu^{ref}_k}{R T}
 \right)
 *   \f]
 *
 *   \f$ K_p \f$ is the simplest form of the equilibrium constant for ideal
 gases. However, it isn't
 *   necessarily the simplest form of the equilibrium constant for other types
 of phases; \f$ K_c \f$ is
 ...
 * @endcode
 *
 *   The model attribute "IdealGas" of the thermo XML element identifies the
 phase as
...
public:
    ...
    //! Construct and initialize an PengRobinsonGasPhase ThermoPhase object
     ...
     *                  empty string.
     */
    PengRobinsonGasPhase(const std::string& inputFile,
                         const std::string& id = "");
    ...
    //! state of the species at the current <I>T</I> and <I>P</I> of the
    //! solution
    /*!
    ...
    //!  of the solution and the reference pressure for the species.
    /*!
     * @param grt     Output vector containing the nondimensional reference
     * state
    ...
    //@}
    /// @name NonVirtual Internal methods to Return References to Reference
    /// State
    /// Thermo
    //@{

    //! Returns a reference to the dimensionless reference state enthalpy
    //! vector.
    /*!
    ...
    // critical properties of the mixture is never used here with eos
    // so a simple mole averaged resutl is returned
    // since the transport class just need critical property of each individual
    // species
    // other method use getCritXXX to set the array

    /// Critical temperature (K).
    virtual doublereal critTemperature() const;
    virtual void getCritTemperature(doublereal* const tc) const;

    /// Critical pressure (Pa).
    virtual doublereal critPressure() const;
    virtual void getCritPressure(doublereal* const pc) const;

    /// Critical volume (m3/kmol)
    virtual doublereal critVolume() const;
    virtual void getCritVolume(doublereal* const vc) const;

    // Critical compressibility (unitless)
    virtual doublereal critCompressibility() const;
    virtual void getCritCompressibility(doublereal* const zc) const;

    /// Critical density (kg/m3).
    // virtual doublereal critDensity() const;

    /// Acentric factor (unitless)
    virtual void getAcentricFactor(doublereal* const om) const;
...
protected:
    ...
    mutable vector_fp sigma;
    mutable vector_fp dipole;

    // criical properties
    mutable vector_fp Tcrit_IJ;
    mutable vector_fp Pcrit_IJ;
    mutable vector_fp Vcrit_IJ;
    mutable vector_fp Zcrit_IJ;
    ...
    mutable doublereal Bm;      // f(Y)
    mutable doublereal dAmdT;   // f(T, Y)
    mutable doublereal d2AmdT2; // f(T, Y)
    mutable doublereal dPdT;    // f(rho, T, Y)
    mutable doublereal dPdV;    // f(rho, T, Y)
    mutable doublereal K1;      // f(rho, T, Y)

    mutable vector_fp dPdN;
    mutable vector_fp dVdN;
    mutable vector_fp dAmdN;
